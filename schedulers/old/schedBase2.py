...
class Scheduler(ABC):
    def __init__(
        ...
        setattr(SequenceGroup, "is_new_seq", False)
    ...
    @abstractmethod
    def _derived_sched(self) -> SchedulerOutputs:
        raise NotImplementedError("_derived_sched")

    def _schedule_chunked_prefill(self) -> SchedulerOutputs:
        return self._derived_sched()
        ...
        for seq_group in self.running:
            if seq_group.is_finished():
                ...
                # Check timeout
                if seq_group.cur_vtime >= timeslice:
        ...
        for seq_group in self.running:
            ...
            if cfs_logger:
                print(
                    f"[CFS][OOM] block_budget[{cur_blocks_budget}] preempting {seq_group.request_id}"
                )
        ...
        while cur_blocks_budget < 0:
            if cfs_logger:
                print(
                    f"[CFS][OOM][hard] block_budget[{cur_blocks_budget}] preempting {seq_group.request_id}"
                )
            # if seq_group.cur_vtime > 0:
    ...
    def _free_finished_seq_group(self, seq_group: SequenceGroup) -> None:
        if seq_group.is_finished():
            print(
                f"[CFS] seq {seq_group.request_id} finished in {seq_group.num_steps} steps"
            )
