...
if [ "$#" -ne 2 ]; then
    echo "Usage: $0 <sched_py_file.sched_class> <oracle_file_path>" >&2
    exit 1
fi
...
#ORACLE=oracle-n10000.txt
#ORACLE=oracle-est-n10000.txt

SCHEDULER=$1
ORACLE=$2

rm ./oracle.txt
ln -s ${ORACLE} ./oracle.txt
...
export VLLM_USE_V1=0; python3 -u -m vllm.entrypoints.openai.api_server \
	...
	--scheduler-cls $SCHEDULER --enable-chunked-prefill \
	--port 8000 --swap-space 16 --disable-log-requests \
