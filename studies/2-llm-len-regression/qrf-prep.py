...
def main():
    ...
    print("ID;prompt_len;prompt_tokens;decode_tokens;n_estimated_decode")
    ...
    for i,l in enumerate(prompts):
        ...
        in_tokens = {k: v.to(device) for k, v in in_tokens.items()}
        n_prompt_tokens = in_tokens["input_ids"].shape[1]
        ...
        with torch.no_grad():
            decode_out = model(**in_tokens, output_hidden_states=True)
